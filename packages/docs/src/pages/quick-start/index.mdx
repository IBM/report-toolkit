...
This "quick start" guide will get you up & running with **<Metadata prop="packageName"/>** in two (2) shakes of a lamb's tail.
...
First things first: to do much of anything with <Metadata prop="packageName"/>, we must generate a report.

<InlineNotification kind="info">
  If you already have a diagnostic report file you intend to use, you can skip
  ahead to the <a href="#redact-secrets-from-a-report">next section</a>.
</InlineNotification>

At the time of this writing (2019-09-23), the diagnostic report functionality is "hidden" behind a flag to the `node` executable: `--experimental-report`. The `process.report` object will not be available unless this flag is present!

The quickest way to generate a report is to "eval" an inline script, like so:
...
1. `--eval "process.report.writeReport()"` -- instead of a script file, execute the double-quoted string as a script, then exit; the script calls [process.report.writeReport()](https://nodejs.org/api/process.html#process_process_report_writereport_filename_err), which writes a report file to disk.

<p>Next, you'll see how <Metadata prop="packageName"/> enables safer storage and transmission of report files.</p>
...
<p><Metadata prop="packageName"/> provides command-line utility named{' '}

<Metadata prop="executable"/>.</p>
...
Let's see what we get. Execute:

```bash
rtk diff report-1.json report-2.json
```

You'll see something like the below (except in technicolor, where available):
...
```bash
node --experimental-report --report-filename \
report-3.json --report-uncaught-exception --eval "throw new Error('oh no')"
```
...
```bash
node --experimental-report --report-filename \
report-4.json --report-uncaught-exception --eval "throw new Error('oh no')"
```
...
```bash
node --experimental-report --report-filename \
report-5.json --report-uncaught-exception --eval "throw new Error('pizza party')"
```
...
You can see that the `sha` property is different, because the exception thrown has a different message.

That wraps up the <Metadata prop="packageName"/> quick start guide! See below for more detailed documentation.
